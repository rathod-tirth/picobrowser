...
export interface FavoritePromptsStorage {
  ...
  updatePrompt: (
    ...
    title: string,
    content: string,
  ) => Promise<FavoritePrompt | undefined>;
  updatePromptTitle: (
    id: number,
    title: string,
  ) => Promise<FavoritePrompt | undefined>;
...
const favoritesStorage: BaseStorage<FavoritesStorage> = createStorage(
  "favorites",
  initialState,
  {
  ...
  return {
    ...
    reorderPrompts: async (
      ...
      await favoritesStorage.set((prev) => {
        ...
        // Find indexes
        const sourceIndex = promptsCopy.findIndex(
          (prompt) => prompt.id === draggedId,
        );
        const targetIndex = promptsCopy.findIndex(
