...
function processSchemaNode(
  ...
  if (node.anyOf) {
    ...
    const processedAnyOf = node.anyOf.map((item) =>
      processSchemaNode(item, definitions),
    );
    ...
    if (key !== "$ref") {
      if (key === "properties" && typeof value === "object" && value !== null) {
        ...
        for (const [propKey, propValue] of Object.entries(value)) {
          result.properties[propKey] = processSchemaNode(
            propValue as JsonSchemaObject,
            definitions,
          );
        }
      } else if (
        ...
        result.items = processSchemaNode(
 ...
 */
export function convertOpenAISchemaToGemini(
  openaiSchema: JsonSchemaObject,
  ensureOrder = false,
): JsonSchemaObject {
  ...
  if (flattenedSchema.properties) {
    geminiSchema.properties = processPropertiesForGemini(
 ...
 */
function processPropertyForGemini(
  property: JsonSchemaObject,
  addPropertyOrdering = false,
): JsonSchemaObject {
  ...
  if (property.type === "object" && property.properties) {
    result.properties = processPropertiesForGemini(
      property.properties,
      addPropertyOrdering,
    );
  ...
  if (property.type === "array" && property.items) {
    result.items = processPropertyForGemini(
      property.items,
      addPropertyOrdering,
    );
    ...
    case "object": {
      if (Array.isArray(value)) {
        ...
        const items = value.map(
          (item) =>
            `${currentIndent}${stringifyCustom(item, currentIndent, baseIndent)}`,
        );
        return `[\n${items.join(",\n")}\n${indent}]`;
      }
      ...
      const properties = keys.map((key) => {
        ...
        const formattedValue = stringifyCustom(
          value[key] as JSONSchemaType,
          currentIndent,
          baseIndent,
        );
