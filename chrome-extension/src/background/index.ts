...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name === "side-panel-connection") {
    ...
    if (
      ...
      senderUrl !== SIDE_PANEL_URL
    ) {
      logger.warning(
        "Blocked unauthorized side-panel-connection",
        senderId,
        senderUrl,
      );
      port.disconnect();
        ...
        switch (message.type) {
          ...
          case "new_task": {
            if (!message.task)
              return port.postMessage({
                type: "error",
                error: t("bg_cmd_newTask_noTask"),
              });
            if (!message.tabId)
              return port.postMessage({
                type: "error",
                error: t("bg_errors_noTabId"),
              });
          ...
          case "follow_up_task": {
            if (!message.task)
              return port.postMessage({
                type: "error",
                error: t("bg_cmd_followUpTask_noTask"),
              });
            if (!message.tabId)
              return port.postMessage({
                type: "error",
            ...
            if (currentExecutor) {
              ...
              const result = await currentExecutor.execute();
              logger.info(
                "follow_up_task execution result",
                message.tabId,
                result,
              );
            } else {
              ...
              logger.info(
                "follow_up_task: executor was cleaned up, can not add follow-up task",
              );
              return port.postMessage({
                type: "error",
                error: t("bg_cmd_followUpTask_cleaned"),
              });
            }
            break;
          }
          ...
          case "cancel_task": {
            if (!currentExecutor)
              return port.postMessage({
                type: "error",
                error: t("bg_errors_noRunningTask"),
              });
          ...
          case "resume_task": {
            if (!currentExecutor)
              return port.postMessage({
                type: "error",
                error: t("bg_cmd_resumeTask_noTask"),
              });
          ...
          case "state": {
            try {
              ...
              const elementsText =
                browserState.elementTree.clickableElementsToString(
              ...
              return port.postMessage({
                type: "success",
                msg: t("bg_cmd_state_printed"),
              });
            } catch (error) {
              logger.error("Failed to get state:", error);
              return port.postMessage({
                type: "error",
                error: t("bg_cmd_state_failed"),
              });
            }
          }
          ...
          case "speech_to_text": {
            try {
              ...
              const speechToTextService =
                await SpeechToTextService.create(providers);
              ...
              const transcribedText =
                await speechToTextService.transcribeAudio(base64Audio);
          ...
          case "replay": {
            ...
            if (!message.historySessionId)
              return port.postMessage({
                type: "error",
                error: t("bg_cmd_replay_noHistory"),
              });
            logger.info(
              "replay",
              message.tabId,
              message.taskId,
              message.historySessionId,
            );
            ...
            try {
              ...
              const result = await currentExecutor.replayHistory(
                message.historySessionId,
              );
          ...
          default:
            return port.postMessage({
              type: "error",
              error: t("errors_cmd_unknown", [message.type]),
            });
        }
      } catch (error) {
