  ...
  async execute(): Promise<void> {
    ...
    try {
      ...
      for (step = 0; step < allowedMaxSteps; step++) {
        ...
        if (navigatorDone) {
          logger.info(
            "🔄 Navigator indicates completion - will be validated by next planner run",
          );
        }
      }
      ...
      if (isCompleted) {
        ...
        const maxStepsError = new MaxStepsReachedError(
          t("exec_errors_maxStepsReached"),
        );
      ...
      if (error instanceof RequestCancelledError) {
        ...
        this.context.emitEvent(
          Actors.SYSTEM,
          ExecutionState.TASK_FAIL,
          t("exec_task_fail", [errorMessage]),
        );

        // Track task failure with detailed error categorization
        const errorCategory = analytics.categorizeError(
      ...
      if (this.generalSettings?.replayHistoricalTasks) {
        ...
        logger.info(
          "Replay historical tasks is disabled, skipping history storage",
        );
      }
    }
  }
  ...
  async replayHistory(
    ...
    try {
      const historyFromStorage =
        await chatHistoryStore.loadAgentStepHistory(sessionId);
      if (!historyFromStorage) {
        throw new Error(t("exec_replay_historyNotFound"));
      }

      const history = JSON.parse(
        historyFromStorage.history,
      ) as AgentStepHistory;
      ...
      this.context.emitEvent(
        Actors.SYSTEM,
        ExecutionState.TASK_FAIL,
        t("exec_replay_fail", [errorMessage]),
      );
    }
