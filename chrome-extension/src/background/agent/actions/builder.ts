...
export class ActionBuilder {
  ...
  buildDefaultActions() {
    ...
    const searchGoogle = new Action(
      async (input: z.infer<typeof searchGoogleActionSchema.schema>) => {
        const context = this.context;
        const intent =
          input.intent || t("act_searchGoogle_start", [input.query]);
    ...
    const goToUrl = new Action(
      async (input: z.infer<typeof goToUrlActionSchema.schema>) => {
    ...
    const goBack = new Action(
      async (input: z.infer<typeof goBackActionSchema.schema>) => {
    ...
    const wait = new Action(
      async (input: z.infer<typeof waitActionSchema.schema>) => {
        ...
        const intent =
          input.intent || t("act_wait_start", [seconds.toString()]);
        this.context.emitEvent(
          Actors.NAVIGATOR,
          ExecutionState.ACT_START,
          intent,
    ...
    const clickElement = new Action(
      async (input: z.infer<typeof clickElementActionSchema.schema>) => {
        const intent =
          input.intent || t("act_click_start", [input.index.toString()]);
        this.context.emitEvent(
          Actors.NAVIGATOR,
          ExecutionState.ACT_START,
          intent,
        ...
        try {
          ...
          const msg = t("act_errors_elementNoLongerAvailable", [
            input.index.toString(),
          ]);
          this.context.emitEvent(
            Actors.NAVIGATOR,
    ...
    const inputText = new Action(
      async (input: z.infer<typeof inputTextActionSchema.schema>) => {
        const intent =
          input.intent || t("act_inputText_start", [input.index.toString()]);
        this.context.emitEvent(
          Actors.NAVIGATOR,
          ExecutionState.ACT_START,
          intent,
    ...
    const switchTab = new Action(
      async (input: z.infer<typeof switchTabActionSchema.schema>) => {
        const intent =
          input.intent || t("act_switchTab_start", [input.tab_id.toString()]);
        this.context.emitEvent(
          Actors.NAVIGATOR,
          ExecutionState.ACT_START,
          intent,
    ...
    const closeTab = new Action(
      async (input: z.infer<typeof closeTabActionSchema.schema>) => {
        const intent =
          input.intent || t("act_closeTab_start", [input.tab_id.toString()]);
        this.context.emitEvent(
          Actors.NAVIGATOR,
          ExecutionState.ACT_START,
          intent,
    ...
    // Scroll to percent
    const scrollToPercent = new Action(
      async (input: z.infer<typeof scrollToPercentActionSchema.schema>) => {
        ...
        if (input.index) {
          ...
          const elementNode = state?.selectorMap.get(input.index);
          if (!elementNode) {
            const errorMsg = t("act_errors_elementNotExist", [
              input.index.toString(),
            ]);
            this.context.emitEvent(
              Actors.NAVIGATOR,
    ...
    // Scroll to top
    const scrollToTop = new Action(
      async (input: z.infer<typeof scrollToTopActionSchema.schema>) => {
    ...
    // Scroll to bottom
    const scrollToBottom = new Action(
      async (input: z.infer<typeof scrollToBottomActionSchema.schema>) => {
    ...
    const previousPage = new Action(
      async (input: z.infer<typeof previousPageActionSchema.schema>) => {
        ...
        if (input.index) {
          ...
          try {
            ...
            if (elementScrollTop === 0) {
              const msg = t("act_errors_alreadyAtTop", [
                input.index.toString(),
              ]);
              this.context.emitEvent(
                Actors.NAVIGATOR,
    ...
    // Scroll to next page
    const nextPage = new Action(
      async (input: z.infer<typeof nextPageActionSchema.schema>) => {
    ...
    const scrollToText = new Action(
      async (input: z.infer<typeof scrollToTextActionSchema.schema>) => {
        const intent =
          input.intent ||
          t("act_scrollToText_start", [input.text, input.nth.toString()]);
        this.context.emitEvent(
          Actors.NAVIGATOR,
          ExecutionState.ACT_START,
          intent,
        ...
        try {
          ...
          const msg = t("act_scrollToText_failed", [
    ...
    const getDropdownOptions = new Action(
      async (input: z.infer<typeof getDropdownOptionsActionSchema.schema>) => {
        const intent =
          input.intent ||
          t("act_getDropdownOptions_start", [input.index.toString()]);
        this.context.emitEvent(
          Actors.NAVIGATOR,
          ExecutionState.ACT_START,
          intent,
        ...
        try {
          ...
          const errorMsg = t("act_getDropdownOptions_failed", [
            error instanceof Error ? error.message : String(error),
          ]);
          this.context.emitEvent(
            Actors.NAVIGATOR,
    ...
    const selectDropdownOption = new Action(
      async (
        ...
        const intent =
          input.intent ||
          t("act_selectDropdownOption_start", [
            input.text,
            input.index.toString(),
          ]);
        this.context.emitEvent(
        ...
        try {
          ...
          const msg = t("act_selectDropdownOption_ok", [
            input.text,
            input.index.toString(),
          ]);
          this.context.emitEvent(Actors.NAVIGATOR, ExecutionState.ACT_OK, msg);
