...
export class MessageManagerSettings {
  ...
  constructor(
    ...
    if (options.messageContext !== undefined)
...
export default class MessageManager {
  ...
  public initTaskMessages(
    ...
    if (
      ...
      this.settings.availableFilePaths.length > 0
    ) {
  ...
  public getMessages(): BaseMessage[] {
    const messages = this.history.messages
      .filter((m) => {
        if (!m.message) {
          console.error(
            `[MessageManager] Filtering out message with undefined message property:`,
            m,
          );
          return false;
        }
    ...
    for (const m of this.history.messages) {
      ...
      if (m.message) {
        ...
        console.error(
          `[MessageManager] Found message with undefined message property:`,
          m,
        );
        logger.debug(
          `Message with undefined message property - Token count: ${m.metadata.tokens}`,
        );
      }
    }
  ...
  public addMessageWithTokens(
    ...
    const metadata: MessageMetadata = new MessageMetadata(
      tokenCount,
      messageType,
    );
