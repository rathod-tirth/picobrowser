  ...
  for (const securityPattern of patterns) {
    try {
      ...
      const regex = new RegExp(
        securityPattern.pattern.source,
        securityPattern.pattern.flags,
      );
      ...
      if (regex.test(sanitized)) {
        ...
        if (sanitized.length !== originalLength) {
          wasModified = true;
          logger.debug(
            `Sanitized ${securityPattern.type}: ${securityPattern.description}`,
          );
        }
      }
    } catch (error) {
  ...
  for (const securityPattern of patterns) {
    try {
      ...
      if (regex.test(content)) {
        detectedThreats.add(securityPattern.type);
        logger.debug(
          `Threat detected: ${securityPattern.type} - ${securityPattern.description}`,
        );
      }
    } catch (error) {
