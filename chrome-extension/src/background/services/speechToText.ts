...
export class SpeechToTextService {
  ...
  static async create(
    ...
    try {
      ...
      logger.info(
        "Found provider for speech-to-text:",
        provider ? "yes" : "no",
        provider?.type,
      );
      ...
      logger.info(
        `Speech-to-text service created with model: ${config.modelName}`,
      );
  ...
  async transcribeAudio(base64Audio: string): Promise<string> {
    try {
      ...
      const transcriptionResponse = await this.llm.invoke([
