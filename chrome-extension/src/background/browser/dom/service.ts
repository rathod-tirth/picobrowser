...
async function _buildDomTree(
  ...
  let mainFramePage = mainFrameResult[0]
    ?.result as unknown as BuildDomTreeResult;
  ...
  if (debugMode && mainFramePage.perfMetrics) {
    logger.debug(
      "DOM Tree Building Performance Metrics (main-frame):",
      mainFramePage.perfMetrics,
    );
  }
  ...
  const visibleIframesFailedLoading =
    _visibleIFramesFailedLoading(mainFramePage);
  const visibleIframesFailedLoadingCount = Object.values(
    visibleIframesFailedLoading,
  ).length;
  if (visibleIframesFailedLoadingCount > 0) {
    const tabFrames = await chrome.webNavigation.getAllFrames({ tabId });
    const subFrames = (tabFrames ?? [])
      .filter((frame) => frame.frameId !== mainFrameResult[0].frameId)
      .sort();
...
async function constructFrameTree(
  ...
  resultPage: BuildDomTreeResult;
}> {
  const parentIframesFailedLoading =
    _visibleIFramesFailedLoading(parentFramePage);
  const failedLoadingFrames = allFramesInfo.filter((frameInfo) => {
    return (
      _locateMatchingIframeNode(parentIframesFailedLoading, frameInfo) != null
    );
  });
  const parentIframesFailedCount = Object.values(
    parentIframesFailedLoading,
  ).length;
  ...
  for (const subFrame of failedLoadingFrames) {
    ...
    const subFramePage = subFrameResult[0]
      ?.result as unknown as BuildDomTreeResult;
    ...
    if (debugMode && subFramePage.perfMetrics) {
      logger.debug(
        "DOM Tree Building Performance Metrics (sub-frame" +
          subFrameResult[0].frameId +
          "):",
    ...
    const iframeNode = _locateMatchingIframeNode(
      parentIframesFailedLoading,
      subFrame,
    );
    if (iframeNode == null) {
      ...
      console.warn(
        "Cannot locate the iframe node for:",
        subFrame,
        "with root element:",
        subFrameRootElement,
      );
    ...
    const childrenIframesFailedLoading =
      _visibleIFramesFailedLoading(subFramePage);
    const childrenIframesFailedCount = Object.values(
      childrenIframesFailedLoading,
    ).length;
...
function _getMaxHighlighIndex(
  result: BuildDomTreeResult,
  priorMaxHighlightIndex?: number,
): number {
  return Math.max(
...
function _locateMatchingIframeNode(
  ...
  const result = Object.values(iframeNodes).find((iframeNode) => {
    ...
    if (strictComparison) {
      ...
      heightMatch =
        heightDifference < 10 ||
        heightDifference / Math.max(frameInfo.computedHeight, frameHeight, 1) <
          0.1;
      const widthDifference = Math.abs(frameInfo.computedWidth - frameWidth);
      widthMatch =
        widthDifference < 10 ||
        widthDifference / Math.max(frameInfo.computedWidth, frameWidth, 1) <
          0.1;
      urlMatch = true;
      titleMatch = true;
    }
...
function _getRawDomTreeNodes(
  ...
  for (const [id, nodeData] of Object.entries(result.map)) {
    if (
      nodeData == null ||
      ("type" in nodeData && nodeData.type === "TEXT_NODE")
    ) {
      continue;
    }
...
function _visibleIFramesFailedLoading(
  result: BuildDomTreeResult,
): Record<string, RawDomElementNode> {
...
async function scriptInjectedFrames(
  ...
  try {
    ...
    return new Map(
      results.map((result) => [result.frameId, result.result || false]),
    );
  } catch (err) {
...
export async function injectBuildDomTreeScripts(tabId: number) {
  try {
    ...
    const frameIdsToInject = Array.from(injectedFrames.keys()).filter(
      (id) => !injectedFrames.get(id),
    );
