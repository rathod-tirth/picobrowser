...
export default class Page {
  ...
  private async _findNearestScrollableElement(
    ...
    try {
      ...
      const docElement =
        (await documentElement.asElement()) as ElementHandle<Element> | null;
      return docElement;
    } catch (error) {
    ...
    if (cacheClickableElementsHashes) {
      ...
      if (
        ...
        for (const domElement of updatedStateClickableElements) {
          const hash =
            await ClickableElementProcessor.hashDomElement(domElement);
          domElement.isNew =
            !this._cachedStateClickableElementsHashes.hashes.has(hash);
        }
      }
      ...
      this._cachedStateClickableElementsHashes =
        new CachedStateClickableElementsHashes(updatedState.url, newHashes);
    }
  ...
  async _updateState(useVision = false, focusElement = -1): Promise<PageState> {
    ...
    try {
      ...
      const [scrollY, visualViewportHeight, scrollHeight] =
  ...
  async scrollToPercent(
    ...
    if (!elementNode) {
      ...
      const scrollableElement =
        await this._findNearestScrollableElement(element);
      if (!scrollableElement) {
        throw new Error(
          `No scrollable ancestor found for element: ${elementNode}`,
        );
  ...
  private async _checkAndHandleNavigation(): Promise<void> {
    ...
    if (
      ...
      try {
        ...
        logger.error(
          `Failed to redirect to safe URL: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
