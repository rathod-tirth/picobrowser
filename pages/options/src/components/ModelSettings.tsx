...
function isOpenAIReasoningModel(modelName: string): boolean {
  ...
  return (
    ...
    (modelNameWithoutProvider.startsWith("gpt-5") &&
      !modelNameWithoutProvider.startsWith("gpt-5-chat"))
  );
}
...
export const ModelSettings = ({ isDarkMode = false }: ModelSettingsProps) => {
  ...
  const [modifiedProviders, setModifiedProviders] = useState<Set<string>>(
    new Set(),
  );
  const [providersFromStorage, setProvidersFromStorage] = useState<Set<string>>(
    new Set(),
  );
  const [selectedModels, setSelectedModels] = useState<
    Record<AgentNameEnum, string>
  >({
  ...
  const [selectedSpeechToTextModel, setSelectedSpeechToTextModel] =
    useState<string>("");
          ...
          if (config) {
            ...
            if (
              ...
              setModelParameters((prev) => ({
                ...
                [agent]: {
                  temperature:
                    config.parameters?.temperature ?? prev[agent].temperature,
            ...
            if (config.reasoningEffort) {
              setReasoningEffort((prev) => ({
                ...prev,
                [agent]: config.reasoningEffort as
                  | "minimal"
                  | "low"
                  | "medium"
                  | "high",
  ...
  useEffect(() => {
    ...
    if (
      newlyAddedProviderRef.current &&
      providers[newlyAddedProviderRef.current]
    ) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !target.closest(".provider-selector-container")
      ) {
      ...
      for (const [provider, config] of Object.entries(storedProviders)) {
        if (config.type === ProviderTypeEnum.AzureOpenAI) {
          ...
          const providerModels =
            config.modelNames ||
            llmProviderModelNames[
              provider as keyof typeof llmProviderModelNames
            ] ||
            [];
          models.push(
  ...
  const handleApiKeyChange = (
    ...
    setProviders((prev) => ({
      ...
      [provider]: {
        ...
        apiKey: apiKey.trim(),
        baseUrl:
          baseUrl !== undefined ? baseUrl.trim() : prev[provider]?.baseUrl,
      },
    }));
  };
  ...
  const removeModel = (provider: string, modelToRemove: string) => {
    ...
    setProviders((prev) => {
      ...
      if (!providerData.modelNames) {
        ...
        const filteredModels = defaultModels.filter(
      ...
      return {
        ...
        [provider]: {
          ...providerData,
          modelNames: providerData.modelNames.filter(
            (model) => model !== modelToRemove,
          ),
        },
      };
  ...
  const getButtonProps = (provider: string) => {
    ...
    if (providerType === ProviderTypeEnum.CustomOpenAI) {
      ...
      hasInput =
        Boolean(config?.apiKey?.trim()) && Boolean(config?.baseUrl?.trim());
  ...
  const handleSave = async (provider: string) => {
    try {
      ...
      await llmProviderStore.setProvider(
        provider,
        configToSave as ProviderConfig,
      );
  ...
  const handleModelChange = async (
    ...
    console.log(
      `[handleModelChange] Setting ${agentName} model: provider=${provider}, model=${model}`,
    );
      ...
      if (model) {
        ...
        if (isOpenAIReasoningModel(modelValue)) {
          ...
          const defaultReasoningEffort =
            agentName === AgentNameEnum.Planner ? "low" : "minimal";
          setReasoningEffort((prev) => ({
            ...prev,
  ...
  const renderModelSelect = (agentName: AgentNameEnum) => (
    ...
    >
      <h3
        className={`mb-2 text-lg font-medium ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}
      >
        {agentName.charAt(0).toUpperCase() + agentName.slice(1)}
      </h3>
      <p
        className={`mb-4 text-sm font-normal ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
      >
            ...
            <div className="flex items-center">
              ...
              <div className="flex flex-1 items-center space-x-2">
                ...
                <div className="flex items-center space-x-2">
                  <span
                    className={`w-12 text-sm ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}
                  >
  ...
  const getSortedProviders = () => {
    ...
    const filteredProviders = Object.entries(providers).filter(
      ([providerId, config]) => {
        // ALSO filter out any provider missing a config or type, to satisfy TS
        if (!config || !config.type) {
          console.warn(
            `Filtering out provider ${providerId} with missing config or type.`,
          );
          return false;
        }
    ...
    return filteredProviders.sort(([keyA, configA], [keyB, configB]) => {
      ...
      const isNewA =
        !providersFromStorage.has(keyA) && modifiedProviders.has(keyA);
      const isNewB =
        !providersFromStorage.has(keyB) && modifiedProviders.has(keyB);
  ...
  const addAzureProvider = () => {
    ...
    const azureProviders = Object.keys(providers).filter(
      (key) =>
        key === ProviderTypeEnum.AzureOpenAI ||
        key.startsWith(`${ProviderTypeEnum.AzureOpenAI}_`),
    );
    ...
    const providerId =
      nextNumber === 1
        ...
        : `${ProviderTypeEnum.AzureOpenAI}_${nextNumber}`;
  ...
  const removeAzureDeployment = (
    provider: string,
    deploymentToRemove: string,
  ) => {
    setModifiedProviders((prev) => new Set(prev).add(provider));
    ...
    setProviders((prev) => {
      ...
      const filteredDeployments = deploymentNames.filter(
        (name) => name !== deploymentToRemove,
      );
  ...
  return (
    <section className="space-y-6">
      ...
      >
        <h2
          className={`mb-4 text-xl font-semibold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}
        >
          ...
          {getSortedProviders().length === 0 ? (
            ...
            getSortedProviders().map(([providerId, providerConfig]) => {
              ...
              if (!providerConfig || !providerConfig.type) {
                console.warn(
                  `Skipping rendering for providerId ${providerId} due to missing config or type`,
                );
              ...
              return (
                ...
                >
                  ...
                  {modifiedProviders.has(providerId) &&
                    !providersFromStorage.has(providerId) && (
                      <div
                        className={`mb-2 text-sm ${isDarkMode ? "text-teal-300" : "text-teal-700"}`}
                      >
                  ...
                  <div className="space-y-3">
                    ...
                    {providerConfig.type === ProviderTypeEnum.CustomOpenAI && (
                      <div className="flex flex-col">
                        ...
                        {nameErrors[providerId] ? (
                          <p
                            className={`ml-20 mt-1 text-xs ${isDarkMode ? "text-red-400" : "text-red-500"}`}
                          >
                            {nameErrors[providerId]}
                          </p>
                        ) : (
                          <p
                            className={`ml-20 mt-1 text-xs ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
                          >
                    ...
                    {(providerConfig.type as ProviderTypeEnum) ===
                      ProviderTypeEnum.AzureOpenAI && (
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          >
                            ...
                            {(providerConfig.azureDeploymentNames || [])
                              .length > 0
                              ? (providerConfig.azureDeploymentNames || []).map(
                  ...
                  {Object.keys(providers).indexOf(providerId) <
                    Object.keys(providers).length - 1 && (
                    <div
                      className={`mt-4 border-t ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}
                    />
                  )}
                </div>
              );
          ...
          <div className="provider-selector-container relative pt-4">
            ...
            >
              ...
              <span className="text-sm">
                {t("options_models_addNewProvider")}
              </span>
            </Button>
              ...
              >
                <div className="py-1">
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleProviderSelection(ProviderTypeEnum.CustomOpenAI)
                    }
                  >
                    <span className="font-medium">
