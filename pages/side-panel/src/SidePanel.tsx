...
const SidePanel = () => {
  ...
  const [hasConfiguredModels, setHasConfiguredModels] = useState<
    boolean | null
  >(null); // null = loading, false = no models, true = has models
  ...
  const appendMessage = useCallback(
    (newMessage: Message, sessionId?: string | null) => {
      ...
      const effectiveSessionId =
        sessionId !== undefined ? sessionId : sessionIdRef.current;
  ...
  const handleReplay = async (historySessionId: string): Promise<void> => {
    try {
      ...
      const historyData =
        await chatHistoryStore.loadAgentStepHistory(historySessionId);
      if (!historyData) {
        appendMessage({
          actor: Actors.SYSTEM,
          content: t(
            "chat_replay_noHistory",
            historySessionId.substring(0, 20),
          ),
          timestamp: Date.now(),
        });
        return;
        ...
        <header className="header relative">
          ...
          <div className="header-icons">
            ...
            <button
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && chrome.runtime.openOptionsPage()
              }
              className={`header-icon ${isDarkMode ? "text-sky-400 hover:text-sky-300" : "text-sky-400 hover:text-sky-500"} cursor-pointer`}
            ...
            {hasConfiguredModels === false && (
              ...
              >
                <div className="max-w-md text-center">
                  ...
                  />
                  <h3
                    className={`mb-2 text-lg font-semibold ${isDarkMode ? "text-sky-200" : "text-sky-700"}`}
                  >
                    {t("welcome_title")}
                  </h3>
                  ...
                  <>
                    ...
                    >
                      <ChatInput
                        ...
                        isDarkMode={isDarkMode}
                        historicalSessionId={
                          isHistoricalSession && replayEnabled
                            ? currentSessionId
                            : null
                        }
