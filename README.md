...
* 🌐 **Open-source Google Chrome Extension** built on **Manifest V3**, using the **NanoBrowser** multi-agent framework for AI-driven web automation.

* 🔒 Establishes an **on-device privacy boundary**: `transformers.js` + **ONNX Runtime Web/WebGPU** run the `openai/privacy-filter` model **locally** to detect and **redact PII** before context is sent to remote LLMs.

* ⚠️ Conventional cloud agents expose **DOM trees, user inputs, and screenshots** to external inference APIs, creating risks for sensitive data such as **names, emails, phone numbers, payment credentials, and authentication tokens**.

* 🛡️ PicoBrowser keeps sensitive values **local** by replacing PII with **ephemeral surrogate tokens**, sending only sanitized structure to the LLM.

## Project Overview & Architectural Vision 🌐

* 🤖 **AI web agents** enable autonomous web scraping, form submission, and complex browser navigation, but typically send **DOM trees, user inputs, and screenshots** to external cloud APIs.

* 🚨 This creates serious **privacy and security risks** when pages contain PII or sensitive information.

* 🏠 **PicoBrowser creates a local security boundary** inside the user's browser, intercepting agent context before it reaches external endpoints.

* 🧠 A **locally hosted neural network** detects sensitive information and replaces it with **temporary surrogate tokens** before external transmission.

* ☁️ Cloud LLMs receive only **sanitized structural context** and generate actions using surrogate tokens.

* 🔄 PicoBrowser **re-hydrates the original values locally** immediately before DOM execution, keeping sensitive information within the user's browser.
...
`Client-Side Inference` | `transformers.js` | `ONNX Runtime Web` | `WebGPU Acceleration` | `openai/privacy-filter` | `Token Classification` | `Named Entity Recognition (NER)` | `PII Detection` | `PII Redaction` | `Keyword Vault`
